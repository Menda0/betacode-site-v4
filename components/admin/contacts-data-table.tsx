...
function createColumns(onView: (contact: ContactRow) => void): ColumnDef<ContactRow>[] {
  return [
    {
    ...
    },
    {
      ...
      header: "Source",
      cell: ({ row }) => (
        <Badge variant="secondary">{getContactSourceLabel(row.original.source)}</Badge>
      ),
    },
    {
      accessorKey: "locale",
...
export function ContactsDataTable({ data }: { data: ContactRow[] }) {
  ...
  const [selectedContact, setSelectedContact] = React.useState<ContactRow | null>(null)
  ...
  const filteredData = React.useMemo(() => {
    ...
    return data.filter((row) => row.source === sourceFilter)
  }, [data, sourceFilter])

  const handleView = React.useCallback((contact: ContactRow) => {
    setSelectedContact(contact)
    setDialogOpen(true)
  }, [])

  const columns = React.useMemo(() => createColumns(handleView), [handleView])
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const { name, email, website, message } = row.original
      return (
        name.toLowerCase().includes(query) ||
        email.toLowerCase().includes(query) ||
        (website?.toLowerCase().includes(query) ?? false) ||
        (message?.toLowerCase().includes(query) ?? false)
      )
    },
  })
  ...
  if (data.length === 0) {
    return (
      <div className="mx-4 rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground lg:mx-6">
        No contacts yet. Submissions from contact forms will appear here.
      </div>
    )
  }
      ...
      <div className="flex w-full flex-col gap-4 px-4 lg:px-6">
        ...
        >
          <TabsList>
            ...
            <TabsTrigger value="betacode-ventures">Ventures</TabsTrigger>
